// https://github.com/MacTuitui/nannou-examples/

use nannou::prelude::bevy_render::render_resource as wgpu;
...
fn model(app: &App) -> Model {
    ...
    let fs_mod_texture =
        shader_from_spirv(&device, Path::new("assets/shaders/tex-simple.frag.spv"));
...
fn update(app: &App, model: &mut Model) {
    ...
    if app.elapsed_frames() <= 1 {
        ...
        let c: Srgba = Color::srgba(0.0, 0.0, 0.0, 0.001);
    ...
    let angle = frac * TAU;
    let c: Srgba = Color::srgba(angle.cos() * 0.5 + 0.5, 0.0, 0.0, 1.0);
    draw.ellipse()
        .radius(radius * (angle.sin() * 0.2 + 1.0))
        .x_y(radius * angle.cos(), radius * angle.sin())
        .color(c);
    let c: Srgba = Color::srgba(0.0, angle.sin() * 0.5 + 0.5, 0.0, 1.0);
    draw.ellipse()
