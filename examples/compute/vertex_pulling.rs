...
fn model(app: &App) -> Model {
    ...
    let particles = create_vertex_storage_buffer(app, size);
...
fn update(app: &App, model: &mut Model) {
    ...
    if new_size > model.buffer_size {
        model.particles = create_vertex_storage_buffer(app, new_size);
...
fn create_vertex_storage_buffer(app: &App, size: u32) -> Handle<ShaderStorageBuffer> {
    ...
    particles.buffer_description.label = Some("vertices");
...
fn create_indirect_params_buffer(app: &App, size: u32) -> Handle<ShaderStorageBuffer> {
    // We populate all of the indirect args in the compute shader, so we just need to create
    // a buffer to store the data
    let mut indirect_params = ShaderStorageBuffer::from(DrawIndirectArgs::default());
